...
public class CommentDAO extends DAO{
	...
	public boolean insertComment(CommentVO cvo) {
		...
		try {
			...
			psmt.setInt(1, cvo.getUserNo());
			psmt.setInt(2, cvo.getBoardNo());
			psmt.setString(3, cvo.getCmtContent());
	...
	public List<CommentVO> selectList(int boardNo){
		...
		try {
			...
			while(rs.next()) {
				...
				cvo.setUserName(rs.getString("user_name"));
				cvo.setCmtContent(rs.getString("cmt_content"));
				list.add(cvo);
			}
			conn.close();
		} catch (SQLException e) {
